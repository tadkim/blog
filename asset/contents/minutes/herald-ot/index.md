...
# Herald design tech 2016
smart design : code + typography
...
- bracket : 통합 개발 환경
    - 다양한 기술 및 라이브러리 활용(p5.js, d3.js등)
...
대개  __Computational__,  __Algorithm__, __Parametaric__, __Generative__ 등의 단어를 사용하는데,
 이것으로는 표현하기 힘들다고 생각. 따라서, 다음과 같이 정리해 보려고 함.
...
5. Typography Case Study
    5.1 MetaFont(1982), Donald Knuth : 사용되는 양에따라 serif에서 sans-serif로 변경되어가는 예.
        5.1.1 MetaFlop(2012) : 웹에서 변수를 조절하면서
        5.1.2. Alexis Reigel : 실제 메타폰트를 사용하고있음
        5.1.3. Type & code
        5.1.4. Code & type
        5.1.5. Adobe Faces : 인터페이스로 이런 폰트를 만들 수 있게 작업을 하고 있다.
    5.2 Computer Modern : 62개의 Parameter가 존재한다
...
## 새로운 기술을 대하는 방식
1. 전통적 현실을 재현
    1.1 The write Brothers - First Flight

    1.2 사진 기법을 이용한 콜라주 기법
        - 이전과 다른 센세이션한 표현기법
        - ex : Grete Stern, Articulos eletioc

    1.3 정연두, 새-B-카메라 (2013)
        - 가상과 현실을 뒤집는 측면에서 의미가 있음

    1.4 구텐베르크 금속활자로 찍은 성경의 페이지(1450년대)
        - 전통적이라는 현실(캘리그래피)을 재현하는 것을 목표로 함.
        - "현실의 재현"사례

    1.5 Garamond 글자꼴(15세기)
...
> 오덕이 되면 사람이 이렇게 됨 ㅋㅋ
    ...
    1.9 Dead History (1990)
     ...
     1.10 Nevile Broday and jon Wo----(1991)
        - 다양한 필터를 남발해서 만든 서체
